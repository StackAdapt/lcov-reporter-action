...
`
		...
		expect(lcov).toEqual([
			{
				...
				branches: {
					...
					found: 5,
					details: [
						{ line: 21, block: 0, branch: 0, taken: 1 },
						{ line: 21, block: 0, branch: 1, taken: 2 },
						{ line: 22, block: 1, branch: 0, taken: 1 },
						{ line: 22, block: 1, branch: 1, taken: 2 },
						{ line: 37, block: 2, branch: 0, taken: 0 },
					],
				},
			},
		])
	...
	it("parse should fail on invalid lcov", async () => {
		await expect(parse("invalid")).rejects.toThrow("Failed to parse string")
	})
	...
	it("percentage should calculate the correct percentage", () => {
		...
		expect(percentage(data)).toBe(75)
	})
})
